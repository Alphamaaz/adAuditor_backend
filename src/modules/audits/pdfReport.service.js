...
const writeHeader = (doc, audit) => {
  doc.font("Helvetica-Bold").fontSize(22).fillColor("#171717").text("AdAuditor Pro Report");
  doc.moveDown(0.25);
...
const writeFooter = (doc) => {
  ...
  for (let index = 0; index < pageRange.count; index += 1) {
    ...
    doc
      ...
      .fillColor("#9ca3af")
      .text(
        `Generated by AdAuditor Pro • Page ${index + 1} of ${pageRange.count}`,
        50,
        doc.page.height - 40,
...
export const generateAuditPdfFile = async ({ audit, version }) => {
  ...
  await new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      bufferPages: true,
      info: {
        Title: `AdAuditor Pro Report - ${audit.adAccount?.name || audit.id}`,
        Author: "AdAuditor Pro",
      },
    });
