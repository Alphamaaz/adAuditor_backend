...
const formatCellValue = (value, label = "", currency = "USD") => {
  ...
  if (money && moneyLikeLabel(label)) {
    return `${String(currency || "USD").toUpperCase()} ${money[1]}${money[2] || ""}`.trim();
  }
...
const renderMasthead = (doc) => {
  ...
  return `<header class="masthead">
    ...
    <div class="mast-meta">
      ...
      <span>Audited by<b>AdAdviser Engine + AI Strategist</b></span>
  ...
  return `<div class="chart"><svg viewBox="0 0 720 ${h}" role="img" aria-label="${escapeHtml(block.caption || "Horizontal bar chart")}">
    ...
    ${rows
      .map((row, i) => {
        ...
        const display = escapeHtml(formatCellValue(row.display ?? row.value, row.label, block.currency));
...
const renderActionPlan = (plan = {}) =>
  plan.rows?.length
    ? `<section class="section">
      <div class="section-rule"><span class="no">AP</span><span class="ln"></span></div>
      <div class="eyebrow">Action plan</div>
      <h2>${escapeHtml(cleanClientText(plan.title))}</h2>
      ${plan.intro ? `<p>${markdown(plan.intro)}</p>` : ""}
      <table><thead><tr><th style="width:52px">Order</th><th>Action</th><th style="width:96px">Effort</th><th class="num" style="width:150px">Expected result</th></tr></thead>
      <tbody>${plan.rows
        .map((row) => `<tr><td class="mono">${escapeHtml(row.order)}</td><td>${markdown(row.action)}</td><td>${escapeHtml(cleanClientText(row.effort))}</td><td class="num ${row.result_tone === "good" ? "highlight" : ""}">${escapeHtml(cleanClientText(row.result))}</td></tr>`)
        .join("")}</tbody></table>
    </section>`
    : "";
...
export const renderReport = (doc) => `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>AdAdviser Performance Audit</title><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,500;9..144,600;9..144,700&family=Inter:wght@400;500;600;700&family=IBM+Plex+Mono:wght@500;600&display=swap" rel="stylesheet"><style>${premiumReportStyles}${embeddedReportOverrides}${premiumPolishOverrides}</style></head><body><div class="sheet">${renderMasthead(doc)}${renderKeyNumbers(doc.key_numbers)}<div class="pad body-wrap">${renderExecutiveSummary(doc.executive_summary)}${(doc.sections || []).map(renderSection).join("")}${renderActionPlan(doc.action_plan)}${renderMethod(doc.method_notes)}<div class="doc-foot"><span><b>AdAdviser</b> · AI-powered PPC audits</span><span>Figures come from verified account data · narrative explains the evidence</span></div></div></div></body></html>`;

export const renderReportForPdf = (doc) => `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>AdAdviser Performance Audit</title><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,500;9..144,600;9..144,700&family=Inter:wght@400;500;600;700&family=IBM+Plex+Mono:wght@500;600&display=swap" rel="stylesheet"><style>${premiumReportStyles}${premiumPolishOverrides}${pdfOverrides}</style></head><body><div class="sheet">${renderMasthead(doc)}${renderKeyNumbers(doc.key_numbers)}<div class="pad body-wrap">${renderExecutiveSummary(doc.executive_summary)}${(doc.sections || []).map(renderSection).join("")}${renderActionPlan(doc.action_plan)}${renderMethod(doc.method_notes)}<div class="doc-foot"><span><b>AdAdviser</b> · AI-powered PPC audits</span><span>Figures come from verified account data · narrative explains the evidence</span></div></div></div></body></html>`;

export const renderAuditPremiumReportHtml = (audit) => renderReport(buildReportDocumentFromAudit(audit));
